...
def nth_missing_number_brute( array : List[int], n : int ):
    missing_number = n

    for num in range(max(array)):
...
def nth_missing_number_optimal( array : List[int], n : int ):
    low = 0
    high = max(array)

    while low < high:
        mid = low + (high-low)//2
        missing_numbers_count = array[mid] - (mid+1)
        if missing_numbers_count >= n: high = mid
        else : low = mid+1
...
array = [1, 3, 5, 6, 8, 10]
n = 4
ans = nth_missing_number_optimal(array, n)
print(f"{n}th number missing in {array} : {ans}")