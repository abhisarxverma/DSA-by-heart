...
// Helper funtion to check if we get the required bouqets on that day
bool check_day( vector<int> array, int bouqets, int flowers, int day ){
    int bouqet_count = 0;
    int flower_count = 0;

    for (auto bloom_day : array){
        if (bloom_day <= day){
            flower_count++;
            if ( flower_count == flowers ) {
                bouqet_count++;
                flower_count = 0;
            }
        }
...
int bouqet_brute( vector<int> array, int bouqets, int flowers ){
    int low = *min_element(array.begin(), array.end());
...
int bouqet_optimal( vector<int> array, int bouqets, int flowers ){
    int low = *min_element(array.begin(), array.end());
    int high = *max_element(array.begin(), array.end());
    ...
    return check_day( array, bouqets, flowers, low) ? low : -1;
}

int main() {
    vector<int> bloom_days = {4, 7, 3, 8, 6};
    int ans = bouqet_brute(bloom_days, 3, 2);
    cout << "Min days : " << ans << endl;
}
