...
def nth_root_brute( number : int, n : int ):
    for i in range((number//n)+2):
        n_power = power(i, n)
...
def nth_root_optimal( number : int, n : int ):
    low = 0
    high = (number//n)+1
    ...
    while low <= high:
        ...
        elif mid_power < number : low = mid+1
        else : high = mid-1
    
    return -1

nth_root = nth_root_optimal( 243, 5 )
print(f"Nth_root : {nth_root}")