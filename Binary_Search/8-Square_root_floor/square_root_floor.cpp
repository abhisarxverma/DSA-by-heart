...
int square_root_floor_brute( int number ){
    int ans = -1;
...
int square_root_floor_optimal( int number ){
    int low = 1;
    int high = (number/2)+1;
    ...
    while ( low <= high ){
        ...
        if (mid_square > number) high = mid-1;
        else low = mid+1;
    }
    return high;
}
...
int main() {
    int square_root = square_root_floor_brute(102);
