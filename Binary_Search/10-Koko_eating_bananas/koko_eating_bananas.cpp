...
int check_eating_time( vector<int> array, int target, int speed ){
    long long ans = 0;
    for ( auto it : array ){
        long long ceil = (it + speed - 1) / speed;
        ans += ceil;
...
int koko_brute( vector<int> array, int target_hour ){
    int low = 1;
...
int koko_optimal ( vector<int> array, int target_hour ){
    int low = 1;
    int high = *max_element(array.begin(), array.end());
...
int main() {
    vector<int> array = {3, 6, 7, 11};
    int ans = koko_optimal( array, 8 );
    cout << "Ans : " << ans << endl;
}