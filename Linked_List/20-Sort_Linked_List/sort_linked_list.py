...
def sort_linked_list_brute( head: Node ):
...
def sort_linked_list_better( head: Node ):
    if not head or not head.next: return head
    ...
    while first:
        ...
        mover = min.next
        while mover:
            if mover.data < min.data: min = mover
            mover = mover.next
        if min != first:
            temp = min.data
            min.data = first.data
...
def merge(left: Node, right: Node):
    dummy = Node(-1)  # Dummy node to simplify merging
    k = dummy  # Pointer to track merging process
    ...
    return dummy.next  # Return the actual merged list

def merge_sort(head: Node):
    if not head or not head.next:
        return head

    middle = get_middle(head)  # Assume get_middle function exists
    left = head
    right = middle.next
