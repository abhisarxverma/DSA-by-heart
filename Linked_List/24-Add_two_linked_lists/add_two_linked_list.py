...
def add_two_linked_lists_brute(head1: Node, head2: Node):
    ...
    while mover1 or mover2:
        ...
        if add < 10:
            carry = 0
            res_mover.next = Node (add)
            res_mover = res_mover.next
        else :
            res_mover.next = Node(add%10)
            carry = add//10
            res_mover = res_mover.next

    if carry: res_mover.next = Node(carry)
    ans = result.next
...
def add_two_linked_lists_recursively(head1: Node, head2: Node, carry=0):
    ...
    res = Node(sum % 10)
    res.next = add_two_linked_lists_recursively(head1, head2, sum // 10)

    return res

def add_two_linked_lists_optimal(head1: Node, head2: Node) :
    head1 = reverse_linked_list(head1)
    head2 = reverse_linked_list(head2)

    result = add_two_linked_lists_recursively(head1, head2)
...
head2 = Node(3, Node(5, Node(7, Node(9))))

print("List 1 : ", end="")
traverse_linked_list(head1)
print("List 2 : ", end="")
traverse_linked_list(head2)
ans = add_two_linked_lists_optimal(head1, head2)
print("Ans : ", end="")
