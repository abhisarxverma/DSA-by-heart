...
def find_missing_repeating_brute( array : List[int], n : int ) -> tuple :
    missing = -1
    repeating = -1
...
def find_missing_repeating_better( array : List[int], n : int ):
    hasharray = [0] * (n+1)
    for num in array:
        hasharray[num] += 1
    missing = -1
...
def find_missing_repeating_better2( array : List[int], n : int ):
    array = sorted(array)
    repeating = -1
    current_sum = 0
    for i in range(n):
        if i < n-1 and array[i] == array[i+1]: repeating = array[i]
        current_sum += array[i]
    
    real_sum = (n*(n+1))//2

    missing = real_sum - current_sum + repeating
    return (missing, repeating)
...
def find_missing_repeating_optimal( array : List[int], n : int ):
    ...
    for i in range(n):
        ...
        S2 +=  array[i] *  array[i]; # Actual sum of squares

    # 4. Compute the differences between actual and expected values
    val1 = S - SN;   # Difference in sums → X - Y (Repeating - Missing)
    val2 = S2 - S2N; # Difference in squares → X² - Y²
...
vec = [8, 6, 1, 7, 5, 2, 3, 8]
ans = find_missing_repeating_optimal(vec, len(vec))
print(ans)