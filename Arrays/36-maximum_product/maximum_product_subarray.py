...
def maximum_product_subarray_brute( array : List[int] ):
...
def maximum_product_subarray_optimal( array : List[int] ):
    maximum_product = float("-inf")
    prefix_product = 1
    suffix_product = 1
    n = len(array)
    for i in range(n):
        ...
        suffix_product *= array[n-1-i]
        maximum_product = max(maximum_product, max(prefix_product, suffix_product))
    
    return maximum_product

array = [2, -1, 5, 7, -2, -9, 10, -6]
maximum_product = maximum_product_subarray_brute(array)
print(f"Maximum product : {maximum_product}")